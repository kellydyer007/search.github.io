...
Purpose: Conduct a science-y like experiment on graph traversal methods relating back to the traveling salesman problem(TSP). The TSP is given a graph, in this case fully connected, what is the shortest path from a starting city to all other cities in the given graph.
...
I mostly used the python search code given in the Russell and Norvig AI:MA textbook. This consisted of a few classes:
  * Graph Class: Used along with Node Class to construct graphs with a given adjacency matrix. 
  * Priority Queue: Implemented priority queue.
  * Problem Class: Abstract class to define problem subclasses.
  * Graph Problem Class: Subclass of Problem where most of the handwritten code is located. The actions and path cost are used to get neighbors in the graph at a node. Also contains all the heuristics needed for A* search:
     * cheapest_edges
     * random_edges
     * primm_mst
     * kruskal_mst
...
Compares the nodes expanded along with the average cost of the path found for each different search algorithm.
...
As you can see, simulated annealing does not have an bars. This is because it is so fast, the scale of the y axis is too large for a bar to appear at y=.002 which is how fast simulated anneal was on larger graphs. This is kind of cool and incredible it runs so fast while there’s no chance I could run MST on large sizes like 25,27, and 30. 
...
In this graph you can see that genetic and simulated annealing algorithm come pretty close to the optimal MST A* path cost. Hill climbing seems to not do very well in estimating the path cost, perhaps because it is not optimal every time. You may be able to get the cost closer to MST A* by doing more random restarts. Similarly with genetic algorithm, you may still be able to lower the delta by running more iterations. Simulated annealing does very well at estimating path cost, but like the other two algorithms, as the node size goes up, the difference in path cost also increases.

# Conclusion

This project was actually very interesting to run and look at the time and path cost results. Using the AI:MA code took awhile to understand, but after some tinkering, I think it works pretty well. It is disappointing not to be able to get MST A*, or any A* search, to run on larger graphs, even after letting it run overnight. Perhaps if I were to attempt this project again, I wouldn’t use Google CoLab and would set up an environment on my PC. Using Google CoLab runs my code on the Google virtual machines, which was problematic because they time you out occasionally for no understood reason, so my size 10 graphs never completed even after 6 hours of runtime. 

Even though I couldn’t get the larger graphs to complete, you can see the incredible logistic increase in the number of nodes expanded between graphs of size 5,7, and 8. With the large jump between sizes 7 and 8, one could understand why size 10 takes so long to run just due to the incredible amount of nodes being expanded. 
